...
# that the merger has occurred between z_this and z_prev; hence we always want to take the ceil and
...
print(np.sum(andromeda_selection))
...
if (z_this in zs_mt[zs_m1]) or (z_this in zs_mt[zs_m2]) or (z_this in zs_mt[zs_m3]):
    ...
    n_merger = count_progenitors(npout, nstart, main_progs, progs, masses_prev, offsets_prev, slabs_prev, mass_ms_lb, mass_ms_hb)
...
print("n_merger m1 sum = ", n_merger_m1.sum())
...
# remove halos with no merger tree information (bad idea because at higher redshifts you remove many objects that may have fulfilled the three-merger requirement already
#n_merger_m1 = n_merger_m1[missing]
#n_merger_m2 = n_merger_m2[missing]
#n_merger_m3 = n_merger_m3[missing]

# total number of mergers per redshift range (sanity check)
print("n_merger m1 sum = ", n_merger_m1.sum())
print("n_merger m2 sum = ", n_merger_m2.sum())
print("n_merger m3 sum = ", n_merger_m3.sum())

# save number of mergers per redshift range
np.save("data/n_merger_m1.npy", n_merger_m1)
np.save("data/n_merger_m2.npy", n_merger_m2)
np.save("data/n_merger_m3.npy", n_merger_m3)
